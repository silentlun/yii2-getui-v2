<?php
namespace silentlun\getui\utils;

use silentlun\getui\GTException;
...
class GTHttpManager
{
    ...
    private static function request($url, $data, $gzip, $method,$headers)
    {
       ...
       // 通过代理访问接口需要在此处配置代理
        ...
        //请求失败有3次重试机会
        $result = self::exeBySetTimes(GTConfig::getHttpTryCount(), $curl);
        return $result;
    }
    ...
    public static function httpRequest($url, $params,$headers, $gzip = false, $method)
    {
        ...
        $result = null;
        try {
            $resp = self::request($url, $data, $gzip,$method,$headers);
